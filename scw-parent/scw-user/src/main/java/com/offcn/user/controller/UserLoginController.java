...
import com.offcn.user.bean.vo.req.UserRegisterVo;
import com.offcn.user.bean.vo.resp.UserRespVo;
import com.offcn.user.commponent.SmsTemplate;
...
/**
 * @author 胡长生
 * @create 2020-10-22 22:00
 */
@RestController
@RequestMapping("/user")
@Api(tags = "用户登录/注册(忘记密码)模块")
@Slf4j
...
//        String code = UUID.randomUUID().toString().substring(0, 4);
        ...
        redisTemplate.opsForValue().set(phoneNo, code, 60 * 15, TimeUnit.DAYS);
        //3、短信发送构造参数
    ...
    public AppResponse<Object> register(UserRegisterVo registVo) {
    ...
    public AppResponse<UserRespVo> login(String username,String password) {
        ...
        //3.经常根据令牌查询用户的id信息
        redisTemplate.opsForValue().set(token,member.getId()+"",2,TimeUnit.DAYS);
