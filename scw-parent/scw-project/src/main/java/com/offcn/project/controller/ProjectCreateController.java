...
import com.offcn.project.vo.req.ProjectBaseInfo;
...
/**
 * @author 胡长生
 * @create 2020-10-24 20:23
 */
@Api(tags = "项目基本模块（创建、保存、项目信息获取等）")
@Slf4j
...
//保存临时项目信息到redis
    ...
    public AppResponse<String> savebaseInfo(ProjectBaseInfo vo) {
        //1.取得redis中之前存储的JSON结构的项目信息
    ...
    @PostMapping("/submit")
    public AppResponse<Object> submit(String accressToken, String projectToken, String ops) {

        //1.前置校验，检查access token
        String memberId = stringRedisTemplate.opsForValue().get(accressToken);
        if(StringUtils.isEmpty(memberId)) {
        ...
        if(!StringUtils.isEmpty(ops)) {
            ...
            if(ops.equals("1")) {
                ...
                //保存草稿
                projectCreateService.saveProjectInfo(statusEnum,storageVo);
                return AppResponse.ok(null);
            } else {
